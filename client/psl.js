...
// ── PSL tiers ──
const PSL_TIERS = [
  { max: 3,        img: './extreme chud plat.PNG', label: '🐷 Extreme Chud',  cls: 'psl-tier-obese'  },
  { max: 4.5,      img: './chud plat.PNG',          label: '😤 Chud',          cls: 'psl-tier-over'   },
  { max: 6.5,      img: null,                        label: '😐 Normie',        cls: 'psl-tier-normal' },
  { max: 8,        img: './prince plat.PNG',         label: '✨ Chadlite',      cls: 'psl-tier-prince' },
  { max: Infinity, img: './king plat.PNG',           label: '👑 Chad',          cls: 'psl-tier-king'   },
];
...
function showResults(data) {
  ...
  // Score number
  document.getElementById('psl-score').textContent   = score.toFixed(1);
  ...
  // Speech bubble
  document.getElementById('psl-bubble').textContent = data.headline || data.roast?.split('.')[0] || '';
